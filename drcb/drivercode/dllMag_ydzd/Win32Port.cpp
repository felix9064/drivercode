...
int CWin32Port::SendData(const char *buffer, int size)
{
	if( !mbOpened || m_hIDComDev == NULL ) return( 0 );
...
{
	PurgeComm(m_hIDComDev, PURGE_TXCLEAR);//清除输出缓冲区
}
...
short CWin32Port::OnceReadString(CString &readstr, short TimeOut)
{//此方法的缺点：不能保证完全接收所有的从硬件发出的数据
    unsigned long TimeCurr, TimeDelay;
	MSG Message;
    TimeCurr = GetTickCount();
...
{//缺点：通过判断最后串或超时来退出循环，否则进入死循环
	...
	while(m_bExitRead)
	{
		...
		//硬件不出异常的话，始终返回TRUE
		BOOL bReadStatus = ReadFile( m_hIDComDev, chr,1, 
			&dwBytesRead,&m_OverlappedRead  ) ;
		if( !bReadStatus )
		{
			if( GetLastError() == ERROR_IO_PENDING )
